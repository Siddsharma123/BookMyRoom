using BookYourResidency.App.Models;
...
namespace BookYourResidency.App.Pages
{
    public class PostPropertyBase : ComponentBase
    {
        ...
        public async Task OnFileInputChangeAsync(InputFileChangeEventArgs e)
        {
            ...
            {
                var image = new Image();
                var resizedImageFile = await img.RequestImageFileAsync(format, 250, 250);
                ...
                image.ImageUrl = img.Name;
                image.Filse = buffer;

                PropertyInputModel.Images.Add(image);
            }
        }
